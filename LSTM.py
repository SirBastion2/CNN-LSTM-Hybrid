...
class SwimSequenceDataset(Dataset):
        ...
        if os.path.isdir(self.video_path):
            ...
            for video_file in video_files:
                ...
                while True:
                    ...
                    frame_padded = resize_and_pad_frame(frame, scale, pad_x, pad_y, new_w, new_h,
...
class FatigueLSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        fatigue_pred = self.fc(lstm_out)
        return fatigue_pred




def train_lstm(lstm_model, dataset, num_epochs=50, learning_rate=1e-3):
    ...
    torch.save(lstm_model.state_dict(), "fatigue_lstm_model.pth")
    print("LSTM model saved to fatigue_lstm_model.pth")
...
def main():
    ...
    video_path = r"D:\SwimmingDataset\Videos\PanZhanle6.mp4"
    ...
    sequence_dataset = SwimSequenceDataset(video_path, cnn_model, transform=transform, target_size=(256, 256), fps=fps)
    lstm_model = FatigueLSTM(input_dim=34, hidden_dim=64, num_layers=2)
    train_lstm(lstm_model, sequence_dataset, num_epochs=20000, learning_rate=1e-3)
